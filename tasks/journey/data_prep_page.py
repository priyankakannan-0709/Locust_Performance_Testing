...
class DataPrepPage(BasePage):
    ...
    def _execute_upload(self):
        ...
        response = self.api.upload(                      # ← delegates to API
            study_id  = self.study_id,
            file_path = file_path,
            name      = self._name("POST", "/api/data-prep/upload")
        )

        if response.status_code != 200:
            logger.error(f"❌  [{self.PAGE_KEY}] Upload failed: "
    ...
    def _execute_download(self):
        ...
        response = self.api.download(                    # ← delegates to API
            study_id = self.study_id,
            name     = self._name("GET", "/api/data-prep/download/{study_id}")
        )
        ...
        # ── STUB — download polling placeholder ────────────────────────────
