...
def _build_html(sla_results: dict, comparison_flag: bool, buckets: dict) -> str:
    ...
    # ── Navigation links ───────────────────────────────────────────────────
    nav_links = """
    <div style="background:#34495e; padding:10px 40px; display:flex; gap:12px;">
        <a href="report.html"
           style="color:#ecf0f1; text-decoration:none; padding:6px 16px;
                  border-radius:4px; background:#2c3e50; font-size:0.88em;
                  border:1px solid #4a6278;">
            📊 Locust Default Report
        </a>
        <a href="page_report.html"
           style="color:#ecf0f1; text-decoration:none; padding:6px 16px;
                  border-radius:4px; background:#2c3e50; font-size:0.88em;
                  border:1px solid #4a6278;">
            📄 Page Report
...
<head>
    ...
    <style>
        ...
        .header p  {{ margin: 6px 0 0 0; color: #aab; font-size: 0.9em; }}
...
def generate_page_report(
    ...
    """
    Generate page-level HTML report with grouped bar chart and doughnut chart.
...
def _build_page_report_html(
    ...
    """Build full HTML string for page report with grouped bar + doughnut charts."""
    ...
    # ── Build grouped bar chart datasets as JS ─────────────────────────────
    chart_datasets = []
    ...
    # ── Checkbox controls for grouped bar ─────────────────────────────────
    checkboxes_html = ""
    for i, profile_name in enumerate(profile_names):
    ...
    # ── Doughnut chart data — all profiles ────────────────────────────────
    doughnut_data = {}
    for profile_name in profile_names:
        ...
        doughnut_data[profile_name] = profile_entries

    # ── Doughnut colours — one per page_key ───────────────────────────────
    doughnut_colour_list = [
        ...
        "rgba(236, 112, 99,  0.85)",   # salmon
    ]

    doughnut_data_js    = json.dumps(doughnut_data)
    doughnut_colours_js = json.dumps(
        doughnut_colour_list[:len(page_keys)]
    )
    doughnut_labels_js  = json.dumps(page_keys)

    # ── Dropdown options — default is first profile (profile_50_rows) ─────
    dropdown_options = "\n".join([
        f'<option value="{p}" {"selected" if i == 0 else ""}>{p}</option>'
    ...
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .chart-section h2 {{ color: #2c3e50; font-size: 1.1em;
                             margin-bottom: 6px; }}
        .chart-subtitle   {{ color: #888; font-size: 0.88em;
                             margin-bottom: 20px; }}
        ...
        /* ── Doughnut controls ── */
        .doughnut-controls {{ display: flex; align-items: center;
                              gap: 12px; margin-bottom: 24px; }}
        .dropdown-label   {{ font-size: 0.88em; font-weight: bold;
                             color: #555; }}
        .profile-dropdown {{ padding: 7px 14px; border-radius: 6px;
                             border: 1px solid #ddd; background: white;
                             font-size: 0.88em; color: #2c3e50;
                             cursor: pointer; min-width: 200px; }}
        .profile-dropdown:focus {{ outline: none; border-color: #3498db; }}

        /* ── Doughnut layout ── */
        .doughnut-layout      {{ display: flex; gap: 48px;
                                 align-items: center; flex-wrap: wrap; }}
        .doughnut-canvas-wrap {{ position: relative;
                                 width: 280px; height: 280px;
                                 flex-shrink: 0; }}
        .doughnut-centre      {{ position: absolute; top: 50%; left: 50%;
                                 transform: translate(-50%, -50%);
                                 text-align: center;
                                 pointer-events: none; }}
        .centre-label         {{ font-size: 0.78em; color: #888;
                                 letter-spacing: 1px; }}
        .centre-value         {{ font-size: 1.5em; font-weight: bold;
                                 color: #2c3e50; margin-top: 4px;
                                 transition: color 0.2s; }}

        /* ── Doughnut breakdown table ── */
        .doughnut-breakdown   {{ flex: 1; min-width: 260px; }}
        .breakdown-row        {{ display: flex; align-items: center;
        ...
        .breakdown-dot        {{ width: 11px; height: 11px;
                                 border-radius: 50%; flex-shrink: 0; }}
        .breakdown-name       {{ flex: 1; font-size: 0.88em; color: #2c3e50; }}
        .breakdown-bar-wrap   {{ width: 100px; height: 6px;
                                 background: #eee; border-radius: 3px;
                                 overflow: hidden; }}
        .breakdown-bar        {{ height: 100%; border-radius: 3px;
                                 transition: width 0.4s ease; }}
        .breakdown-ms         {{ font-size: 0.88em; font-weight: bold;
                                 color: #2c3e50; min-width: 72px;
                                 text-align: right; }}
        .breakdown-pct        {{ font-size: 0.82em; color: #888;
...
<body>
    ...
    <div class="content">
        ...
        <div class="chart-section">
            <h2>🍩 Journey Time Distribution by Profile</h2>
            <p class="chart-subtitle">
                Where does the user spend most time across all pages?
            </p>

            <!-- ── Profile dropdown ── -->
            <div class="doughnut-controls">
                <label class="dropdown-label">Profile:</label>
                <select
                    id       = "profileDropdown"
                    class    = "profile-dropdown"
                    onchange = "updateDoughnut(this.value)"
                >
                    {dropdown_options}
                </select>
            </div>

            <!-- ── Doughnut + breakdown side by side ── -->
            <div class="doughnut-layout">

                <!-- ── Canvas ── -->
                <div class="doughnut-canvas-wrap">
                    <canvas id="doughnutChart"></canvas>
                    <div class="doughnut-centre" id="doughnutCentre">
                        <div class="centre-label" id="centreLabel">Total</div>
                        <div class="centre-value" id="centreTotal">0ms</div>
                    </div>
                </div>

                <!-- ── Breakdown table ── -->
                <div class="doughnut-breakdown" id="doughnutBreakdown"></div>
    ...
    <script>
        // ── Grouped bar — all datasets from Python ────────────────────────
        const allDatasets = [
            {chart_datasets_js}
        ];

        // ── Initialise grouped bar chart ──────────────────────────────────
        const ctx = document
        ...
        const doughnutData    = {doughnut_data_js};
        const doughnutColours = {doughnut_colours_js};
        const doughnutLabels  = {doughnut_labels_js};
        ...
        const doughnutChart = new Chart(dCtx, {{
            ...
            options: {{
                ...
                onHover: function(event, elements) {{
                    updateCentreLabel(elements);
                }}
            }}
        }});

        // ── Update centre label on hover ──────────────────────────────────
        function updateCentreLabel(elements) {{
            const centreVal   = document.getElementById('centreTotal');
            const centreLbl   = document.getElementById('centreLabel');
            const currentData = doughnutChart.data.datasets[0].data;
            ...
            if (elements && elements.length > 0) {{
                ...
                const pct   = total > 0
                    ...
                    : 0;
                centreVal.textContent = value + 'ms';
                centreVal.style.color = doughnutColours[idx];
                centreLbl.textContent = doughnutLabels[idx]
                    .replace('_page', '') + ' · ' + pct + '%';
            }} else {{
                centreVal.textContent = total + 'ms';
                centreVal.style.color = '#2c3e50';
                centreLbl.textContent = 'Total';
            }}
        }}

        // ── Update doughnut when profile dropdown changes ─────────────────
        function updateDoughnut(profileName) {{
            const entries = doughnutData[profileName] || [];
            ...
            // ── Reset centre to total ─────────────────────────────────────
            document.getElementById('centreTotal').textContent = total + 'ms';
            document.getElementById('centreTotal').style.color = '#2c3e50';
            document.getElementById('centreLabel').textContent = 'Total';

            // ── Rebuild breakdown table ───────────────────────────────────
            const breakdown = document.getElementById('doughnutBreakdown');
            breakdown.innerHTML = entries.map((entry, i) => {{
                const pct      = total > 0
                    ? ((entry.value / total) * 100).toFixed(1)
                    : 0;
                const colour   = doughnutColours[i % doughnutColours.length];
                const pageName = entry.page.replace('_page', '');
                return `
                <div class="breakdown-row"
                     onmouseenter="highlightSegment(${{i}})"
                     onmouseleave="resetSegments()">
                    <div class="breakdown-dot"
                         style="background:${{colour}}"></div>
                    <div class="breakdown-name">${{pageName}}</div>
                    <div class="breakdown-bar-wrap">
                        <div class="breakdown-bar"
                             style="width:${{pct}}%;
                                    background:${{colour}}"></div>
                    </div>
                    <div class="breakdown-ms">${{entry.value}}ms</div>
                    <div class="breakdown-pct">${{pct}}%</div>
                </div>`;
            }}).join('');
        }}

        // ── Highlight doughnut segment when hovering breakdown row ────────
        function highlightSegment(index) {{
            ...
            // ── Update centre ─────────────────────────────────────────────
            document.getElementById('centreTotal').textContent = value + 'ms';
            document.getElementById('centreTotal').style.color =
                doughnutColours[index];
            document.getElementById('centreLabel').textContent =
                doughnutLabels[index].replace('_page', '') + ' · ' + pct + '%';

            // ── Dim other segments ────────────────────────────────────────
            doughnutChart.data.datasets[0].backgroundColor =
                doughnutColours.map((c, i) =>
                    i === index ? c : c.replace('0.85', '0.2')
                );
        ...
        function resetSegments() {{
            ...
            doughnutChart.data.datasets[0].backgroundColor = doughnutColours;
            doughnutChart.update('none');
        }}

        // ── Toggle endpoint detail ────────────────────────────────────────
        function toggleDetail(sectionId, btn) {{
            const detail = document.getElementById('detail_' + sectionId);
        ...
        updateDoughnut('{default_profile}');
